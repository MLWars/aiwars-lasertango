//! The MCP server for the Laser Tango referee. A faithful copy of the chess
//! warden's `mcp.rs`, typed to `Match<LaserTango>`: a SINGLE bearer-gated `/mcp`
//! endpoint serves all agents; the caller's seat is resolved from its bearer
//! token. The four tools (`get_state`, `legal_moves`, `make_move`, `resign`) are
//! unchanged — they call the game-agnostic `Match` methods, so the move string is
//! just a corridor move (e.g. `advance:gap`) instead of a UCI move.
...
use crate::lasertango::LaserTango;
use aiwars_mcp_warden::game::Match;

/// Shared, mutex-guarded match state shared across all agents' requests.
pub type SharedMatch = Arc<Mutex<Match<LaserTango>>>;

/// `make_move` tool arguments.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct MakeMoveArgs {
    /// The corridor move, one of your legal moves (e.g. "advance:gap",
    /// "wait:beat", "slide:under").
    pub mv: String,
...
impl RefereeHandler {
    #[tool(
        description = "Get the current corridor state: your segment and distance to the door, the rival, the current beat, whether the beam ahead is LIT or DARK, status, winner, and your legal moves.",
    ...
    #[tool(
        description = "List the moves you may make now (advance:gap / wait:beat / slide:under) and whether it is your turn, with the current ply.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Commit a corridor move. Pass the ply you saw as expected_ply. advance:gap steps one segment (trips if the beam ahead is LIT); wait:beat holds and is always safe; slide:under skips two segments fast (trips unless both beams ahead are dark). Returns the new state on success; an error (no change) if illegal, out of turn, or stale.",
    ...
    #[tool(
        description = "Resign the crossing (forfeit). Your rival is awarded the win.",
        annotations(read_only_hint = false, destructive_hint = false, open_world_hint = false)
    )]
...
impl ServerHandler for RefereeHandler {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "AIWars Laser Tango referee. Call get_state / legal_moves to read the beat and whether \
             the beam ahead is LIT, make_move to commit a corridor move (with the ply you saw): \
             advance:gap steps through the gap, wait:beat holds, slide:under skips two segments. \
             Step into a lit beam and you TRIP out; reach the exit door first to BREACH and win. \
             Your seat is identified by your bearer token; you cannot act as your rival.",
        )
    }
}
