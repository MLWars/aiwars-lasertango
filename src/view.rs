...
//! the chess warden's `view.rs`, typed to `Match<LaserTango>`. Serves
//! `/state.json` (a pure read of the match) plus the baked SPA from `view_dir`.
...
pub struct ViewState {
    pub game: Arc<Mutex<Match<LaserTango>>>,
}

/// `GET /state.json` — the live corridor JSON the SPA polls. A pure read.
...
mod tests {
    ...
    fn match_fixture() -> Arc<Mutex<Match<LaserTango>>> {
        let handles = vec!["tempo".to_string(), "blitz".to_string()];
        let m = Match::<LaserTango>::new(handles, &serde_json::json!({ "seed": 7 })).unwrap();
        Arc::new(Mutex::new(m))
    }
    ...
    #[tokio::test]
    async fn state_json_is_a_readable_corridor() {
        let resp = router()
        ...
        assert_eq!(v["game"], "lasertango");
        assert!(v["runners"].is_array());
    }
